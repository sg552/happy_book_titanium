...
我们的开发中, 原则是先做ios端, 再做Android端. 前面做的差不多之后, 我们会发现一个现象: 程序明明在
ios 上运行的很好,但是在Android中却不行:

现象 | 原因 | 解决办法
:---:|:---:|:---:
屏幕是空白的 | 安卓下的字体默认是白色的  |  在各个Label中,指定 color: 'black'
返回按钮的背景色是灰色的 | 安卓下的按钮,默认背景色是灰色| 指定: backgroundColor: 'white'
每一行之间都没有一个灰色的线,表示 List | 安卓默认就是没有的 |
背景色也不完整. 有的宽些,有的窄些. | 安卓默认就是这样的
